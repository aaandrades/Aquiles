...
ap.add_argument('-c', '--config', help = 'path to yolo config file', default='custom_stairs/yolov3-tiny.cfg') ##Yolo configuration CFG
ap.add_argument('-w', '--weights', help = 'path to yolo pre-trained weights', default='custom_stairs/yolov3-tiny_last.weights') ##Yolo COnfiguration Weights
ap.add_argument('-cl', '--classes', help = 'path to text file containing class names',default='custom_stairs/objects.names')
args = ap.parse_args()
...
def draw_pred(img, class_id, confidence, x, y, x_plus_w, y_plus_h):
    ...
    label = ('%.2f' % confidence) #Regular Expression to take the first two digits
...
window_title= "Aquiles Detector - Stairs"   
...
while cv2.waitKey(1) < 0:
    ...
    objects = ct.update(boxes,class_ids,image)
